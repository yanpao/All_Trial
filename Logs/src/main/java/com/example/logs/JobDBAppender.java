...
public class JobDBAppender extends DBAppenderBase<ILoggingEvent> {
    ...
    private static final int EXECUTOR_ID_INDEX = 10;
    private static final int EXECUTOR_IP_INDEX = 11;
    ...
    private static String buildInsertSQL() {
        return "INSERT INTO job_log" +
        "(timestmp, message, level_string, logger_name, thread_name," +
                "caller_filename, caller_class, caller_method, caller_line, executor_id, executor_ip)" +
        "VALUES(?,?,?,?,?,?,?,?,?,?,?)";
    }

    private void bindLoggingEventWithInsertStatement(PreparedStatement stmt, ILoggingEvent event) throws SQLException {
        stmt.setLong(TIME_INDEX, event.getTimeStamp());
    ...
    private void bindCallerDataWithPreparedStatement(PreparedStatement stmt, StackTraceElement[] callerDataArray) throws SQLException {
        ...
        stmt.setString(EXECUTOR_ID_INDEX, "one");
        stmt.setString(EXECUTOR_IP_INDEX, "192.168.56.11");
    }
